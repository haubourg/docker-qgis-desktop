...
```bash
QGIS_VERSION=4.x
cp start_qgis.sh ~/apps/QGIS4/qgis_docker_${QGIS_VERSION}.sh
```

```fish
set QGIS_VERSION 4.x
cp start_qgis.sh ~/apps/QGIS4/qgis_docker_{$QGIS_VERSION}.sh
```
...
```
cp launchers/qgis.desktop ~/.local/share/applications/docker-qgis-4.x.desktop
...
`docker build --no-cache -t ghcr.io/haubourg/qgis_local:4.x ./è

Push to registry

`export CR_PAT=githubtokenhere`
`echo $CR_PAT | docker login ghcr.io -u haubourg --password-stdin`
`docker push ghcr.io/haubourg/qgis_local:4.x`
...
`find . -not -path '*/\.git/*'  -exec sed -i  's/3.34/4.x/g' '{}' \;`
